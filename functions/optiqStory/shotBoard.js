// ─── OPTIQ STORY — THE SHOT BOARD ───────────────────────────────────────────
//
// The story sandbox's own copy. Same machinery as the ad swarm's shot board, and
// deliberately a COPY rather than a shared import: these two boxes share no code
// so that tuning how an ad is photographed can never change how a story is. If
// you are changing a prompt in here for a story, do not "also fix" the ad's.
//
// The only thing that differs today is that a story has no product and no
// packaging to keep consistent, so `branded` defaults false — everything else is
// identical, and it is expected to stay that way until a story genuinely needs
// something an ad does not.
//
// ════════════════════════════════════════════════════════════════════════════
// THE HIERARCHY
// ════════════════════════════════════════════════════════════════════════════
 ...
 */
const FRAMED_PROMPT_MIN = 1000;
const FRAMED_PROMPT_MAX = 1500;
...
The ONLY appearance you may mention is appearance that CHANGES during these ${sceneSeconds} seconds — someone gets soaked, a shirt tears, a screen lights up — because a still cannot show a change. Say what changes, in a clause, and nothing more.

═══ HOW TO WRITE IT ═══

Plain headings, in this order: WHAT HAPPENS (the timestamped beats) · DIALOGUE (with voices) · SOUND · CAMERA. No preamble, no restatement of the story, no summary at the end — but the blocks themselves are as long as they need to be, and SOUND is usually the longest of them. Write it as a working call sheet for a crew who are standing in the place and can already see it, and who need to be told everything they cannot see — because that is exactly the situation the video model is now in.
...
function framedPromptBrief({ scene, shots = [], sceneSeconds = 10 }) {
  const board = (shots || [])
    .filter((s) => s.url)
    .map(
      (shot, i) =>
        `Setup ${i + 1} — ${shot.time}${shot.label ? ` — ${shot.label}` : ""}: ${oneLine(shot.camera, 200)}${
