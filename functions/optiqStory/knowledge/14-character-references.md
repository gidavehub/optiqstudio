...
> **Story sandbox note.** An original short film collects no brand materials — the
> wizard never asks for them — so the ONLY images a scene ever carries are the
> character sheets this module describes. There is no logo to place and no
> product plate to attach. If a director later attaches a reference of their own
> in the script editor, the same correctness rule governs it:

- A **place** goes only on scenes set there.
- A **person** goes only on scenes they are in.
- An **object** goes only on scenes it is physically in frame in — and it is a story element, never a product to be presented.
- Anything genuinely unplaceable is left off. Over-attaching a reference is worse than not using it, because an attachment is an instruction to put that thing in the frame.
