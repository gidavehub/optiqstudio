...
═══ THIS IS LONG FORM — ${Math.round(seconds / 60)} MINUTES — AND IT NEEDS A SECOND ENGINE ═══

${scenes} scenes is not a short film with more scenes in it. The five obligations
above are the SPINE, and at this length a spine alone leaves a slack middle: a
turn at scene ${turnBy} and a climax at scene ${climaxFrom} means roughly ${Math.max(
          1,
          climaxFrom - turnBy
        )} scenes of
"escalate", and escalation repeated ${Math.max(1, climaxFrom - turnBy)} times without new
information is the same scene ${Math.max(1, climaxFrom - turnBy)} times. That is the
failure to design against, and it is the only one this length introduces.

So this film ALSO carries:

  A MIDPOINT, around scene ${Math.max(3, Math.round(scenes * 0.5))}. Not another
  escalation — a REFRAME. Something the audience already saw means something
  different now: the favour was a trap, the stranger was expected, the debt was
  already paid, the person they are protecting does not want protecting. Everything
  before it should be re-readable in a new light, and everything after it plays
  under new rules. Tag it "turn"; a long film is allowed two.

  A SECOND WANT. The thing they wanted at scene ${Math.max(2, Math.round(scenes * 0.2))}
  is not the thing they want at scene ${Math.max(4, Math.round(scenes * 0.8))}. Show the
  swap happening on screen rather than announcing it.

  A REAL COST, PAID. Somewhere after the midpoint, the character loses something
  the audience watched them care about, and it does not come back. Long form earns
  its ending by spending; a film that reaches ten minutes with everyone intact has
  been narrating, not dramatising.

  MORE PEOPLE, EACH WANTING SOMETHING. Two people cannot carry ${scenes} scenes of
  event. Bring in a third and a fourth with their own wants that COLLIDE with the
  lead's — not helpers, not obstacles, people. The best long-form escalation is
  somebody else's plan working.

  A FALSE ENDING before the real one, around scene ${Math.max(
    5,
    Math.round(scenes * 0.85)
  )}. It looks
  resolved. It is not. Then the climax lands on top of it.

TWISTS ARE STRUCTURAL, NOT COSMETIC. A twist is not a surprising fact revealed; it
is a REVERSAL OF WHO IS IN CONTROL. Ask, at every turn, "who has the power now?" —
if the answer is the same as it was three scenes ago, nothing has actually turned.
Reach for the version nobody sees coming and then make it inevitable in hindsight:
plant it early, in a detail that reads as texture the first time.

AND KEEP IT MOVING. Every one of the ${scenes} scenes is still an EVENT with its
full beat count. Length is not permission to breathe — a slow scene in a
${Math.round(seconds / 60)}-minute film is not a rest, it is a viewer leaving.`
      : scenes >= 12
...
function storyStructureViolations(storyline, { numScenes } = {}) {
  ...
  // ── The long-form gates ───────────────────────────────────────────────────
  //
  // Only for films that are genuinely long. At 30+ scenes the five-obligation
  // spine is satisfiable by a story that turns once and then escalates fifteen
  // times, and "escalate" fifteen times is one scene fifteen times — the exact
  // failure this length introduces, and the one that is invisible to every gate
  // above because each individual scene passes.
  if (total >= 24) {
    const turns = acts.filter((a) => a === "turn").length;
    if (turns < 2) {
      violations.push(
        `This is a ${Math.round((total * 10) / 60)}-minute film with ${turns} turn in it. At ${total} scenes one ` +
          `turn leaves a middle that can only repeat itself. Add a MIDPOINT around scene ` +
          `${Math.max(3, Math.round(total * 0.5))}, tagged "turn": not another escalation, a REFRAME that makes ` +
          `everything before it mean something different and puts everything after it under new rules.`
      );
    }

    // The longest unbroken run of one obligation. A long film is allowed a lot of
    // escalation; it is not allowed nine scenes in a row that all serve the same
    // purpose, because nothing distinguishes them from each other.
    let longestRun = 0;
    let runAct = null;
    let run = 0;
    let previous = null;
    for (const act of acts) {
      run = act && act === previous ? run + 1 : 1;
      previous = act;
      if (run > longestRun) {
        longestRun = run;
        runAct = act;
    ...
    const runCeiling = Math.max(4, Math.round(total * 0.3));
    if (longestRun > runCeiling) {
      violations.push(
        `${longestRun} consecutive scenes are all tagged "${runAct}" — more than the ${runCeiling} this film's ` +
          `length allows. A run that long is the same scene told ${longestRun} times with different framing. Break ` +
          `it up: something must change hands, arrive, be discovered or be lost inside that stretch, and the scenes ` +
          `that do it are tagged accordingly.`
      );
    }
  }
