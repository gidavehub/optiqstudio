 ...
 *
 * ON A PHOTOGRAPHED FILM, A SCENE WITH NO FRAMES ATTACHES NOTHING. There is
 * deliberately no fallback to the character sheets or the director's uploads, and
 * this is a hard rule, not an oversight: those pictures exist to build the BOARD,
 * and the board is the only thing the video model is ever shown. A studio
 * portrait handed to the video model is the grey-backdrop contamination this
 * whole system replaced, and a half-photographed film that quietly reverts to it
 * is worse than one that renders from its words — because it looks like it is
 * working. Such a scene is surfaced on the board screen with a Retry instead.
 *
 * ON EVERY OTHER KIND OF FILM, this returns the scene's own reference images,
 * unchanged. That branch is load-bearing. The version of this function at tag
 * `shot-board-experiment` returned board stills for ALL films, and since only a
 * photographed film has a board, every ad and every ordinary story silently began
 * rendering with NOTHING attached — the render still succeeded, it just quietly
 * stopped holding faces. Do not "simplify" the fallback away.
 *
...
function renderAttachments(project, sceneIndex) {
  ...
  if (!board) {
    // Not a photographed film. Whatever the swarm attached to this scene —
    // character sheets, the director's uploads — is what rides along, exactly as
    // it did before the board existed.
    const images = project?.sceneImages || {};
    return images[sceneIndex] ?? images[String(sceneIndex)] ?? [];
  }
