...
|---|---|---|
| 6 Ã— 10s | 60s | **The default.** Enough for a complete story. |
| 9 Ã— 10s | 90s | Room for a real second act |
| 12 Ã— 10s | 120s | Two strands, or a larger cast |
| 18 Ã— 10s | 180s | The ceiling. A BIGGER story, never a slower one. |

> **Story sandbox note.** Every scene in this system is exactly 10 seconds — the
> run-time the director paid for divided by ten. The mixed-length rows this table
> used to carry (8s scenes, 10s reserved for a motion-graphics logo outro) belong
> to the ad swarm. There are no motion-graphics scenes and no outros here.

**The arc, regardless of count** (this is the STORY arc — see Part XV, which is
the authority on it; the commercial funnel this section used to describe does not
apply to an original short film):
1. **The open** (something is already wrong; the question is raised)
2. **The want** (what they are trying to get, shown physically)
3. **The turn** (it stops being the story we thought it was)
4. **The escalation** (pressure rises, options close)
5. **The climax** (the biggest moment: the confrontation, the decision, the reveal)
6. **The landing** (the outcome, on camera — what is different now)
