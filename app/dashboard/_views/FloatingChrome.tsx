...
// brand there. The agent room and the board shots are the same case: each has a
// real top bar of its own. The script editor ("manual") and every other page
// keep them.
...
export default function FloatingChrome() {
  ...
  // Three surfaces own their own top bar and carry the brand themselves, so the
  // pills would just collide with it: the timeline editor, the agent room and the
  // board shots. Note the timeline test is anchored — a `startsWith` would also
  ...
  const inBoardShots = /^\/dashboard\/project\/[^/]+\/board$/.test(path);
  if (inTimelineEditor || inAgentRoom || inBoardShots) return null;
