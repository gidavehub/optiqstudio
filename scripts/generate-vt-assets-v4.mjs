/**
 * MASTER ASSET GENERATION SCRIPT (V4) - Virtual Teacher Rohey
 * 
 * Runs from the Optiq repository workspace (utilizing Optiq's authenticated Cloud Functions).
 * Calls the Optiq developer API endpoints over HTTP to synthesize audios and generate videos.
 * 
 * Saves all generated files directly into the virtualteacher workspace:
 *   c:\Projects\virtualteacher\public\media\
 * 
 * Run: node scripts/generate-vt-assets-v4.mjs
 */

import { mkdir, writeFile, readFile, access } from "fs/promises";
import path from "path";

const API_KEY = "optiq_live_1feac90d9b883bc482d3d697498cbbe52679cd18c5d1cc56";
const BASE_URL = "https://us-east4-davelabs-tools.cloudfunctions.net";
...
async function generateTTS(audio) {
  console.log(`\n[Cloud Function Call] Generating TTS: ${audio.file}`);
  
  const res = await fetch(`${BASE_URL}/apiGenerateTTS`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok) {
    throw new Error(`TTS Cloud Function error: ${data.error || res.statusText}`);
  }

  console.log(`  ✓ Cloud Function success. ID: ${data.id}. Storage URL: ${data.url}`);
  console.log(`  … Downloading audio file over HTTP...`);
  const destPath = path.join(OUT, audio.file);
  await downloadFile(data.url, destPath);
  console.log(`  ✓ Saved directly to: ${destPath}`);
}

async function generateVideo(vid, imageBase64) {
  console.log(`\n[Cloud Function Call] Requesting video generation: ${vid.file}`);
  
  const res = await fetch(`${BASE_URL}/apiGenerateVideo`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${API_KEY}`
  ...
  if (!res.ok) {
    throw new Error(`Video generation Cloud Function error: ${initData.error || res.statusText}`);
  }

  const generationId = initData.id;
  console.log(`  ✓ Initialized. Generation ID: ${generationId}. Status: ${initData.status}`);

  // Poll video status
  console.log(`  … Polling video generation status via apiGetVideoStatus...`);
  const maxAttempts = 60; // 10 minutes max
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    await new Promise(r => setTimeout(r, 12000)); // wait 12s between checks
    
    try {
      const statusRes = await fetch(`${BASE_URL}/apiGetVideoStatus?id=${generationId}`, {
        headers: {
          Authorization: `Bearer ${API_KEY}`
        }
      });
      ...
      if (!statusRes.ok) {
        console.warn(`    ⚠ Polling check failed (HTTP ${statusRes.status}): ${statusData.error}`);
        continue;
      }

      console.log(`    [Poll ${attempt}/${maxAttempts}] Status: ${statusData.status}`);

      if (statusData.status === "succeeded") {
        console.log(`  ✓ Video successfully generated! Storage URL: ${statusData.videoUrl}`);
        console.log(`  … Downloading video file over HTTP...`);
        const destPath = path.join(OUT, vid.file);
        await downloadFile(statusData.videoUrl, destPath);
        console.log(`  ✓ Saved directly to: ${destPath}`);
        return;
      } else if (statusData.status === "failed") {
        throw new Error(`Cloud generation reported failed: ${statusData.error || "unknown error"}`);
      }
    } catch (pollErr) {
...
async function main() {
  ...
  console.log(`\n======================================================`);
  console.log(`=== CLOUD FUNCTION MEDIA PIPELINE RUNNING (V4) ===`);
  console.log(`======================================================`);

  console.log(`\n--- STEP 1: Generating Audio TTS via Cloud Functions ---`);
  for (const audio of AUDIOS) {
    try {
      if (await exists(audio.file)) {
        console.log(`  ✓ Audio ${audio.file} already exists, skipping.`);
        continue;
      }
  ...
  console.log(`\n--- STEP 2: Generating Consistent Videos via Cloud Functions ---`);
  for (const vid of VIDEOS) {
    try {
      ...
      // Small 20s cushion to avoid API storming
  ...
  console.log("\n======================================================");
  console.log("Cloud Function Media generation process complete.");
  console.log("======================================================");
}
