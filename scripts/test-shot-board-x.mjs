...
// ONE brain, because there is one board. It was built for every film once and
// reverted on cost; it lives in the experimental long-form story alone now, and
// scripts/test-video-types.ts fails if a copy reappears in any other box.
const brain = brainNs.default ?? brainNs;
...
// ── 10. This box sells nothing, and it says who is who ──────────────────

console.log("\nThe experimental sandbox:");
check(
  "it never asks for a product — nothing in this film is being sold",
  !brain.worldDirective({ numScenes: 30 }).includes("including the product")
);
check(
  "it carries the anti-copying mandate",
  brain.worldDirective({ numScenes: 30 }).includes("THE EXAMPLES ARE FORMAT, NEVER CONTENT") &&
    brain.shotDesignDirective({ environment: null }).includes("THE EXAMPLES ARE FORMAT, NEVER CONTENT")
);
check(
  "the shooting brief asks for a CAST KEY",
  brain.framedPromptDirective({}).includes("CAST KEY")
);
check(
  "and it tells the model the frames win a disagreement",
  /THE FRAMES ARE RIGHT/i.test(brain.framedPromptDirective({}))
);
